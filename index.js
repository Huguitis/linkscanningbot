 // Hi why are you looking at this :flushed:
 ...
 const bot = new aoijs.Bot({
   ...
   suppressAllErrors: setting.SuppressPackageErrors,
   sharding: setting.Sharding,
   shardCount: setting.ShardCount,
   errorMessage: "",
   intents: setting.BotIntents,
   respondOnEdit: setting.RespondOnEdit,    
   database:{
        type:"default",
        path: "./Database/",
        tables:["LinkScanningBotDB"],
    }
 });

bot.onMessage({
  respondToBots: setting.RespondToBots
})
bot.onMessageUpdate()

const loader = new aoijs.LoadCommands(bot)
loader.load(bot.cmd,__dirname +"/Utils/")

bot.status({
  text: setting.BotStatus,
...
BotLogsChannelID: setting.BotLogsChannelID,
StatusType: setting.StatusType,
SuccessEmoji: setting.SuccessEmoji,
ErrorEmoji: setting.ErrorEmoji,
LoadingEmoji: setting.LoadingEmoji,
ApiKey: setting.ApiKey,
BotOwners: setting.BotOwners,
ServerLinkScan: setting.LinkScanning,
ServerLanguage: setting.BotLanguage,
RespondToBots: setting.RespondToBots,
CooldownBetweenScans: setting.CooldownBetweenScans,
...
name: "SecureLink",
code:`
$editMessage[$getGlobalUserVar[BadLinksMessageID];{newEmbed:{description:$getVar[SuccessEmoji] **__$if[$getVar[ServerLanguage]==Español;No se han encontrado amenazas;$if[$getVar[ServerLanguage]==English;No malware found;Keine Malware gefunden]].__
$if[$getVar[ServerLanguage]==Español;Sospechoso;$if[$getVar[ServerLanguage]==English;Suspicious;Verdächtig]]: $jsonRequest[https://ipqualityscore.com/api/json/url/$getVar[ApiKey]/$replaceText[$splitText[1];.-.;//];suspicious;]**}{color:GREEN}}]
$textSplit[$replaceText[$advancedTextSplit[$advancedTextSplit[$message;//;2]; ;1];//;.-.];/]
`
})
...
- **Spamming:** $getObjectProperty[spamming]
- **Malware:** $getObjectProperty[malware]
- **Phishing:** $getObjectProperty[phishing]}{color:RED}}]

$createObject[$jsonRequest[https://ipqualityscore.com/api/json/url/$getVar[ApiKey]/$replaceText[$splitText[1];.-.;//]]]
$textSplit[$replaceText[$advancedTextSplit[$advancedTextSplit[$message;//;2]; ;1];//;.-.];/]

$if[$getVar[DeleteIfMaliciousLink]!=false;{execute:DeleteIfMaliciousLink};]
`
})

bot.awaitedCommand({
name: "BanIfMaliciousLink",
code:`
$ban[$authorID]
`
})
...
$onlyForIDs[759796588842450944;$getVar[BotOwners];]
`
})